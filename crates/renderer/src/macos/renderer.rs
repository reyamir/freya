...
use crate::{
    devtools::Devtools,
    macos::state::{create_surface, CreatedState, NotCreatedState, WindowState},
    HoveredNode, LaunchConfig,
};

const WHEEL_SPEED_MODIFIER: f32 = 53.0;

/// Desktop renderer using Skia, Meta and Winit
        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                if let Some(drawable) = metal_layer.next_drawable() {
                    ...
                    let mut surface = create_surface(window, metal_layer, skia);
